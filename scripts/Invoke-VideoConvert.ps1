...
foreach ($d in @($LogsDir, $FailedDir, $ConvertedDir, $ConfigDir)) {
...
if (-not $FfmpegPath)       { $FfmpegPath       = Join-Path $RepoRoot "tools\ffmpeg\bin\ffmpeg.exe" }
if (-not $HandBrakeCliPath) { $HandBrakeCliPath = Join-Path $RepoRoot "tools\handbrake\HandBrakeCLI.exe" }

if (-not $FileBotPath) {
    $FileBotPath = Join-Path $RepoRoot "tools\filebot\filebot.cmd"
    if (-not (Test-Path -LiteralPath $FileBotPath)) { $FileBotPath = Join-Path $RepoRoot "tools\filebot\FileBot.exe" }
}
...
function Update-CompletedIndexEntry {
    ...
    if (-not $Timestamp) { $Timestamp = (Get-Date).ToString("s") }
...
function Add-CompletedRecord {
    ...
    Update-CompletedIndexEntry -InputPath $InputPath -Status $Status -FileSizeBytes ([long]$id.FileSizeBytes) -LastWriteTimeUtc ([string]$id.LastWriteTimeUtc) -OutputPath $OutputPath -Notes $Notes -Timestamp ([string]$rowObj.Timestamp)
}

function Write-CompletedSnapshot {
    if ($DryRun) { return }

    Ensure-CompletedCsvReady -Path $CompletedCsvPath
    $rows = foreach ($k in ($script:CompletedIndex.Keys | Sort-Object)) { $script:CompletedIndex[$k] }

    $rows |
        Select-Object Timestamp,InputPath,OutputPath,Status,Notes,FileSizeBytes,LastWriteTimeUtc |
        Export-Csv -Path $CompletedCsvPath -NoTypeInformation -Encoding UTF8
}
...
function New-HardwareProbeResult {
    $help = Get-HandBrakeEncodersText
    $gpuNames = Get-MachineVideoControllers

    $hasNvenc = ($help -match "nvenc_h264")
    $hasVce   = ($help -match "vce_h264")
    $selected = if ($hasNvenc) { "nvenc_h264" } elseif ($hasVce) { "vce_h264" } else { "x264" }
    ...
    return [pscustomobject]@{
        ...
        GpuNames           = @($gpuNames)
        SupportsNvencH264  = [bool]$hasNvenc
        SupportsAmdVceH264 = [bool]$hasVce
        SelectedEncoder    = $selected
    }
}

function Get-HardwareCachePath {
    return Join-Path $ConfigDir ("hardware-profile-" + $env:COMPUTERNAME + ".json")
}

function Get-SelectedEncoder {
    param(
        [ValidateSet("Auto","NVENC","AMDVCE","X264")]
        [string]$Mode,
        [switch]$RefreshCache
    ...
    if (-not $RefreshCache -and (Test-Path -LiteralPath $cachePath)) {
        try {
            $probe = Get-Content -LiteralPath $cachePath -Raw | ConvertFrom-Json
            if ($probe -and $probe.HandBrakePath -eq $HandBrakeCliPath -and $probe.SelectedEncoder) {
                Write-Log "Using cached hardware profile: $cachePath" "INFO" "DarkGray"
                return [string]$probe.SelectedEncoder
            }
        }
        catch {
            Write-Log "Hardware profile unreadable, rebuilding cache." "WARN" "Yellow"
        }
    }

    $probe = New-HardwareProbeResult
    if (-not $DryRun) {
        $probe | ConvertTo-Json -Depth 5 | Out-File -LiteralPath $cachePath -Encoding UTF8
    }
    $gpuSummary = "Unknown"
    if ($probe.GpuNames -and $probe.GpuNames.Count -gt 0) { $gpuSummary = ($probe.GpuNames -join "; ") }
    Write-Log ("Hardware probe complete. GPUs: {0}" -f $gpuSummary) "INFO" "DarkGray"

    return [string]$probe.SelectedEncoder
}

$SelectedHbEncoder = Get-SelectedEncoder -Mode $EncoderMode -RefreshCache:$RefreshHardwareCache
Write-Log "HandBrake encoder selected: $SelectedHbEncoder" "INFO" "Cyan"

if ($ProbeHardwareOnly) {
    Write-Log "ProbeHardwareOnly requested; exiting after hardware test." "INFO" "Cyan"
    return
}
    ...
    function Get-VideoInfo {
        ...
        try {
            ...
            if ($aObj.streams) {
                foreach ($a in $aObj.streams) {
                    $channels = 0
                    if ($a.channels) { $channels = [int]$a.channels }
                    $aud += [pscustomobject]@{ Codec=([string]$a.codec_name); Channels=$channels }
                }
            }
            $r.Audio = $aud
...
Write-CompletedSnapshot
Write-Log "Completed CSV snapshot refreshed: $CompletedCsvPath" "INFO" "DarkGray"
